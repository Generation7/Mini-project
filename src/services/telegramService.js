...
function resolveOwnedClass(user, parsed) {
  ...
  if (!owned.length) {
    return {
      error: 'You haven\'t created a class yet. Say "create a class called X" first.'
    };
  }
  ...
  if (parsed.className) {
    ...
    if (!match) {
      return {
        error: `❌ I couldn't find a class you created called *${parsed.className}*.`
      };
    }
  ...
  if (owned.length === 1) {
    return { class: owned[0] };
  }

  return {
    error: `You've created more than one class — which one? (${owned.map(c => c.name).join(', ')})`
  };
}
    ...
    function filterByDateWindow(items, dateField, parsed) {
      if (parsed.date) {
        return {
          filtered: items.filter(
            i => i[dateField] === parsed.date
          ),
          windowLabel: parsed.date
        };
      }
      ...
      if (parsed.withinDays) {
        const cutoff = new Date(
          now.getTime() +
          Number(parsed.withinDays) * 24 * 60 * 60 * 1000
        )
          .toISOString()
          .slice(0, 10);
...
Today's date is ${todayDate}.`
    ...
    try {
      ...
      if (!user) {
        return "Sorry, I'm having trouble accessing your account. Please try again.";
      }
      ...
      switch (parsed.action) {
            ...
            l =>
              l.courseCode
                ...
                parsed.courseCode
                  .replace(/\s/g, '')
                  .toLowerCase() &&
              l.lectureDay.toLowerCase() ===
                parsed.lectureDay.toLowerCase()
          );

          if (!lecture) {
            return `❌ I couldn't find *${parsed.courseCode}* on *${parsed.lectureDay}*.`;
          }
            ...
            l =>
              l.courseCode
                ...
                parsed.courseCode
                  ...
                  .toLowerCase() &&
              l.lectureDay.toLowerCase() ===
                parsed.oldLectureDay.toLowerCase()
          );

          if (!lecture) {
            return `❌ I couldn't find *${parsed.courseCode}* on *${parsed.oldLectureDay}*.`;
          }

          lectureService.updateLecture(
            lecture.id,
            {
              lectureDay: parsed.newLectureDay,
              lectureTime: parsed.newLectureTime
            }
          );
        ...
        case 'LIST_LECTURES': {
          ...
          const day =
            parsed.day ? parsed.day.trim() : null;
          ...
          if (!lectures.length) {
            return "You have no lectures yet. Send me a photo of your timetable!";
          }

          if (day && !filtered.length) {
            return `You have no lectures on *${day}*.`;
          }

          const label = day
            ? `📚 *Your Lectures on ${day}:*`
            : `📚 *Your Lectures:*`;

          return `${label}\n${filtered
            .map(
              l =>
                `• ${l.courseCode} - ${l.lectureDay} at ${l.lectureTime}`
            )
            .join('\n')}`;
        }
        ...
        case 'LIST_ASSIGNMENTS': {
          const pending =
            assignmentService.getPendingAssignments(user.id);
          ...
          const list =
            window ? window.filtered : pending;

          if (window && !list.length) {
            return `🎉 Nothing due on *${window.windowLabel}*.`;
          }

          const label = window
            ? `📝 *Assignments Due (${window.windowLabel}):*`
            : `📝 *Your Pending Assignments:*`;

          return `${label}\n${list
            .map(
              a =>
                `• *${a.courseCode}* - ${a.title}\n  📅 Due: ${a.dueDate} at ${a.due_time || '23:59'}`
            )
            .join('\n')}`;
        }
        ...
        case 'COMPLETE_ASSIGNMENT': {
          const assignment =
            assignmentService.markAssignmentDone(
              user.id,
              parsed.courseCode
            );

          if (!assignment) {
            return `❌ I couldn't find a pending assignment for *${parsed.courseCode}*.`;
          }
        ...
        case 'DELETE_ASSIGNMENT': {
          ...
          if (!assignment) {
            return `❌ I couldn't find an assignment for *${parsed.courseCode}*.`;
          }
        ...
        case 'LIST_EXAMS': {
          const upcoming =
            examService.getUpcomingExams(user.id);
          ...
          const list =
            window ? window.filtered : upcoming;

          if (window && !list.length) {
            return `🎉 No exams on *${window.windowLabel}*.`;
          }

          const label = window
            ? `🎓 *Exams (${window.windowLabel}):*`
            : `🎓 *Your Upcoming Exams:*`;
        ...
        case 'COMPLETE_EXAM': {
          const exam =
            examService.markExamDone(
              user.id,
              parsed.courseCode
            );

          if (!exam) {
            return `❌ I couldn't find an upcoming exam for *${parsed.courseCode}*.`;
          }
        ...
        case 'DELETE_EXAM': {
          ...
          if (!exam) {
            return `❌ I couldn't find an exam for *${parsed.courseCode}*.`;
          }
        ...
        case 'CLEAR_LECTURES': {
          ...
          if (!allLectures.length) {
            return "You don't have any lectures to clear.";
          }

          allLectures.forEach(
            l => lectureService.deleteLecture(l.id)
          );
        ...
        case 'CLEAR_ASSIGNMENTS': {
          ...
          if (!pending.length) {
            return "You don't have any pending assignments to clear.";
          }
        ...
        case 'CLEAR_EXAMS': {
          ...
          if (!upcoming.length) {
            return "You don't have any upcoming exams to clear.";
          }
        ...
        case 'JOIN_CLASS': {
          ...
          if (result.error === 'already_member') {
            return `You're already in *${result.class.name}*!`;
          }
        ...
        case 'LIST_CLASSES': {
          const myClasses =
            classService.getUserClasses(user.id);
          ...
          return `🏫 *Your Classes:*\n${myClasses
            .map(
              c =>
                `• ${c.name}${c.isCreator ? ` (you created this — code: *${c.joinCode}*)` : ''}`
            )
            .join('\n')}`;
        }
        ...
        case 'LIST_CLASS_UPDATES': {
          const pending =
            classService.getPendingItemsForUser(user.id);

          if (!pending.length) {
            return '🎉 No new class updates right now.';
          }
          ...
          const lines =
            pending.map(item => {
              ...
              if (item.type === 'lecture') {
                return `#${item.id} 📚 ${p.courseCode} - ${p.lectureDay} at ${p.lectureTime}`;
              }

              if (item.type === 'assignment') {
                return `#${item.id} 📝 ${p.courseCode} - ${p.title} (due ${p.dueDate} ${p.dueTime})`;
              }

              if (item.type === 'exam') {
                return `#${item.id} 🎓 ${p.courseCode} exam ${p.examDate} at ${p.examTime}${p.venue ? ` (${p.venue})` : ''}`;
              }
        ...
        case 'ACCEPT_CLASS_ITEM': {
          const result =
            classService.acceptClassItem(
              parsed.itemId,
              user.id
            );

          if (result.error === 'not_found') {
            return `❌ I couldn't find update #${parsed.itemId}.`;
          }

          if (result.error === 'already_accepted') {
            return `You've already accepted update #${result.item.id}.`;
          }
...
async function processMessage(chatId, userMessage, user) {
  const history =
    user
      ? conversationService.getRecentMessages(user.id)
      : [];
  ...
  if (user) {
    try {
      ...
      console.error(
        `Failed to save conversation history for user ${user.id}: ${err.message}`
      );
    }
  }
...
function startTelegramBot() {
  ...
  bot.onText(
    ...
    async (msg, match) => {
      ...
      try {
        ...
        if (token) {
          const user =
            userService.findByValidTelegramLinkToken(
              token
            );
          ...
          userService.linkTelegramChatId(
            user.id,
            chatId
          );
        ...
        const existing =
          userService.findByTelegramChatId(
            chatId
          );
        ...
        logger.error(
          `/start handler failed for chat ${chatId}: ${err.message}`
        );
      }
    }
  );
  ...
  async function requireLinkedUser(chatId) {
    ...
    if (!user) {
      try {
        ...
        logger.error(
          `requireLinkedUser notice failed for chat ${chatId}: ${err.message}`
        );
      }
  ...
  bot.on(
    ...
    async (msg) => {
      ...
      const user =
        await requireLinkedUser(chatId);

      if (!user) return;

      try {
        await safeSendChatAction(
          chatId,
          'typing'
        );
        ...
        await sendMessageWithRetry(
          bot,
          chatId,
          "📸 Got your timetable! Let me read it..."
        );
...
Be thorough - check every single cell in the timetable carefully.`
        ...
        if (!jsonMatch) {
          return await sendMessageWithRetry(
            bot,
            chatId,
            "Sorry, I couldn't read the timetable clearly. Try a clearer photo!"
          );
        }
        ...
        try {
          await sendMessageWithRetry(
            bot,
            chatId,
            "Sorry, I had trouble reading that image. Please try again!"
          );
        } catch (sendErr) {
          logger.error(
            `Photo error notice failed for chat ${chatId}: ${sendErr.message}`
          );
        }
      }
    }
    ...
    async (msg) => {
      ...
      try {
        ...
        try {
          ...
          // Telegram can reject AI/user-generated text because Markdown
          // contains special characters that Telegram tries to parse.
          // Detect the actual entity-parser error and resend as plain text.
          ...
          if (isMarkdownParseError) {
            ...
            console.warn(
              `Markdown parse failed for chat ${chatId}; retrying as plain text.`
            );
        ...
        try {
          ...
          logger.error(
            `Error notice failed for chat ${chatId}: ${sendErr.message}`
          );
        }
      }
    }
